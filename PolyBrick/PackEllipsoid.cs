    ...
    class PackEllipsoid
    {
        ...
        public void pack(Grid grid)
        {
                                 ...
                                 for (int i = 0; i < cell.Count; i++)
                                 {
                                     ...
                                     for (int j = i + 1; j < cell.Count; j++)
                                     {
                                         ...
                                         //if (Ellipsoid.BroadPhaseCollision(ellipsoid_i,ellipsoid_j))
                                         {
                                             ...
                                             ApplySeparationForcesToEllipsoid(ellipsoid_i, ellipsoid_j, separate_forces, near_ellipsoids, index_i, index_j, rimDistance);
                                     ...
                                     for (int k = 0; k < neighbors.Count; k++)
                                     {
                                         ...
                                         //if (Ellipsoid.BroadPhaseCollision(ellipsoid_i,ellipsoid_k))
                                         {
                                             ...
                                             ApplySeparationForcesToEllipsoid(ellipsoid_i, ellipsoid_k, separate_forces, near_ellipsoids, index_i, index_k, rimDistance);
                                             //collision_index.Add(index_i); //Debug
            ...
            for (int i = EGlobals.EXISTING_POINTS.Count; i < this.ellipsoids.Count; i++)
            {
                ...
                List<Ellipsoid> prev_list = grid.GetOneCell(ei);
                prev_list.Remove(ei);
        ...
        public void ApplySeparationForcesToEllipsoid(Ellipsoid ellipsoid_i, Ellipsoid ellipsoid_j, List<Vector3d> separate_forces, List<int> near_ellipsoids, int index_i, int index_j, double rimDistance)
        {
